...
import com.rasturize.anticheat.utils.world.BlockData;
...
import com.rasturize.anticheat.utils.packet.WrappedPacket;
...
public class Helper {
	...
	private static Class<?> entity = WrappedPacket.c("net.minecraft.server." + WrappedPacket.VERSION + ".Entity");
	private static Class<?> craftEntity = WrappedPacket.c("org.bukkit.craftbukkit." + WrappedPacket.VERSION + ".entity.CraftEntity");
	...
	public static List<Block> getBlocksNearby(CollisionHandler handler, SimpleCollisionBox collisionBox) {
		try {
			return handler.getBlocks().stream().filter(b -> b.getType() != Material.AIR
					&& BlockData.getData(b.getType()).getBox(b, handler.getPlayerData().protocolVersion)
